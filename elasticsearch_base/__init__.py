...
def image_convert_to_read_json(self, value, record, use_name_get=True):
    return f"{record.get_base_url()}/web/image/{self.model_name}/{record.id}/{self.name}"
...
fields.Image.convert_to_read_json = image_convert_to_read_json
